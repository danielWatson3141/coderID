...
def plot_average_roc_auc_curve(roc_averages):
    ...
    plt.plot(
        ...
        label="macro average ROC curve (area = {0:0.4f})".format(roc_averages["unweighted"]["AUC"]),
        linestyle=":",
    )
    # Plot weighted average ROC
    plt.plot(
        roc_averages["weighted"]["fpr"], roc_averages["weighted"]["tpr"], color="teal", lw=lw,
        label="weighted average ROC curve (area = {0:0.4f})".format(roc_averages["weighted"]["AUC"]),
        linestyle=":",
    )
...
def plot_roc_auc_curves(results, directory="plots", session_name=""):
    ...
    for i, (author, metrics) in enumerate(results.items()):
        plt.plot(
            ...
            label="ROC curve for {0:s} (area = {1:0.4f})".format(author, metrics[author]["AUC"]),
        )
    plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
