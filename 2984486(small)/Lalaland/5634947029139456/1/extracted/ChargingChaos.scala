...
object ChargingChaos {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 1 to cases) {
      ...
      val flow = inputFiles.next().split(' ').map(java.lang.Long.parseLong(_,2)).toSet
      val devices = inputFiles.next().split(' ').map(java.lang.Long.parseLong(_,2)).toSet

      def valid(switch:Long): Boolean =
      {
        for (device <- devices)
      ...
      var minSwitch : Long = Int.MaxValue
      for (device <- devices)
        for (flowOption <- flow)
        {
