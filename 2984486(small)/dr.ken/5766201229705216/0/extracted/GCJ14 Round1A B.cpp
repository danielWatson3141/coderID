...
int rec(int r, int v) {
    ...
    if (child.size() == 0) { res = 0; }
        ...
        for (int p = 0; p < child.size(); ++p) {
            for (int q = p+1; q < child.size(); ++q) {
                int tmp = num[v] - 1 - num[child[p]] - num[child[q]] + dp[r][child[p]] + dp[r][child[q]];
                chmin(res, tmp);
            }
        }
    }
...
int main() {
    freopen( "/Users/macuser/Dropbox/Contest/B-small-attempt0.in", "r", stdin );
