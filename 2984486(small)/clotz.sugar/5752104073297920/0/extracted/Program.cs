        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCase = 0; testCase < TestCase; testCase++) {
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCase, N, l);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            }
            string Solve(int testCase, int N, string l) {
                if (testCase == -1) {
                }
                List<int> ns = (from s in l.Split(' ') select int.Parse(s)).ToList();
                long score = 0;
                for (int i = 0; i < 1000; i++) {
                    if (ns[i] > 550) {
                        score += (ns[i] - 550) * (i - 500);
                    }
                }
                long limit = 1350000;
                if (score < limit) {
