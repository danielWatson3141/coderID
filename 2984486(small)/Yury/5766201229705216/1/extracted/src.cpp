...
typedef vector<vector<uint>>    Graph;
...
void input_t::read(istream& in) {
    ...
    //range(varr v : g) v.shrink_to_fit();
}
...
void solver_t::solve() {
    ...
    range(ini, N) {
        ...
        while(st.size() > 1) {
            ...
            valr node = in.g.at(fr.n);
            if(fr.i < node.size()) {
                val j = node.at(fr.i++);

                if(mark.at(j)) continue;
                mark.at(j) = true;
            ...
            uint    d = 1;
            if(fr.mas.size() >= 2) {
                partial_sort(fr.mas.begin(), fr.mas.begin()+2, fr.mas.end(), std::greater<uint>());
                d = fr.mas.at(0) + fr.mas.at(1) + 1;
            }

            st.pop_back();
            st.back().mas.push_back(d);
        }

        ASSERT(st.at(0).mas.size() == 1);

        uint d = st.at(0).mas.at(0);
