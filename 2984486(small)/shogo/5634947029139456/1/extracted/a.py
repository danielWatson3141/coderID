...
def hoge(A, B, mask, cnt):
    ...
    A0 = [a | (mask-1) for a in A if a&mask == 0]
    A1 = [a | (mask-1) for a in A if a&mask != 0]
    B0 = [b | (mask-1) for b in B if b&mask == 0]
    B1 = [b | (mask-1) for b in B if b&mask != 0]

    # don't flip
    if len(A0) == len(B0):
        A0.sort()
        B0.sort()
        if tuple(A0) == tuple(B0):
            hoge(A, B, mask >> 1, cnt)

    # flip
    if len(A0) == len(B1):
        A0 = [a^mask for a in A0]
        A0.sort()
        B1.sort()
        if tuple(A0) == tuple(B1):
