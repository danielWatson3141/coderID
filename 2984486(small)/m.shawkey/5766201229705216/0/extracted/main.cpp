...
int main()
{
	...
	for(int z=1;z<=t;z++)
	{
		...
		for(int i=1;i<=n;i++)
		{
			if(ans==-1 || ans>res[0][i]) ans=res[0][i];
		}
...
int get_ans(int p,int c)
{
	...
	if(tov.size()==0) res[p][c]=0;
	else if(tov.size()==1) res[p][c]=get_del(c,tov[0]);
	else
	{
		int m=0;
		for(int i=0;i<tov.size();i++) m+=get_del(c,tov[i]);
		int ans=-1;
		int t1;
		for(int i=0;i<tov.size();i++)
		{
			for(int j=i+1;j<tov.size();j++)
			{
				t1=m-get_del(c,tov[i])-get_del(c,tov[j])+get_ans(c,tov[i])+get_ans(c,tov[j]);
				if(ans==-1 || t1<ans) ans=t1;
			}
		}
		res[p][c]=ans;
	}
...
int get_del(int p,int c)
{
	if(del[p][c]==-1) 
	{
		...
		{
			if(tree[c][i]==p) continue;
			del[p][c]+=get_del(c,tree[c][i]);
		}
	}
	return del[p][c];
}
