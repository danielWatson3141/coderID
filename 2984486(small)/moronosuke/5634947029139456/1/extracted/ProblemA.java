...
public class ProblemA {
	...
	private static String solveIt(int n, int l, Set<Long> initialSetting, Set<Long> requiredSetting) {
		...
		for (long i : buildPossibleMoves(initialSetting, requiredSetting)) {
	...
	private static Set<Long> buildPossibleMoves(Set<Long> initialSetting, Set<Long> requiredSetting) {
		Set<Long> possibleMoves = new HashSet<>();
		for (long initial : initialSetting) {
			for (long required : requiredSetting) {
				possibleMoves.add(initial ^ required);
			}
		}
