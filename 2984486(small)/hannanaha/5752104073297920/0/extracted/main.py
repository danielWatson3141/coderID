...
@memoizeit
def bad_exact_prob(N):
    n = decimal.Decimal(N)
    print (((n-1) ** n) - n + 1 ) / ((n ** n) * (n - 2))
    print 1/n

def good_shuffle(N=1000):
    perm = range(N)
    for k in xrange(N):
...
def analyze(perm):
    is_ahead = [v < i for i, v in enumerate(perm)]
    is_ahead = [1 if v else 0 for v in is_ahead]
        
    #print is_ahead
    return "GOOD" if sum(is_ahead) > 485 else "BAD" 
    
def analyze_avg(perm):
    diffs = [i - v for i, v in enumerate(perm)]
    mean = 1.0 * sum(diffs)/len(diffs) 
    print mean
    
def analyze_exact(perm):
    exact = [1 if i == v else 0 for i, v in enumerate(perm)]
    print 1.0 * sum(exact) / len(perm)
...
if __name__ == '__main__':
    #print sum(analyze(good_shuffle(1000)) for _ in range(120))
    #print sum(analyze(bad_shuffle(1000)) for _ in range(120))
    
    #analyze(bad_shuffle(1000))
    #bad_exact_prob(1000)
    
    PROBLEM_NO = "C"
#    main("sample.in")
    main("E:\\%s-small-attempt0.in" % PROBLEM_NO)
#    main("E:\\%s-large.in" % PROBLEM_NO)
