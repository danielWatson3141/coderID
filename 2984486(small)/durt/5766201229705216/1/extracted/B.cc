...
int mem1[MAX_NODES+1][MAX_NODES+1];
int mem2[MAX_NODES+1][MAX_NODES+1];
...
int compute_total_nodes(int root, int parent) {
	...
	if (mem2[root][parent] != -1)
		return mem2[root][parent];
...
int min_delete_given_root(int root, int parent) {
	...
	if (mem1[root][parent] != -1)
		return mem1[root][parent];
