...
int get(int v, int prev) {
	pair<int, int> dd(v, prev);
	if (d.count(dd)) return d[dd];

	vector<int> l(2, 0);
	forn(i, g[v].size()) {
		int u = g[v][i];
		if (u != prev) l.pb(get(u, v));
	}

	sort(rall(l));
	if (l[1] == 0) l[0] = 0;
	return d[dd] = l[1] + l[0] + 1;
}

int stupid(int ans = -1) {
    return ans;
}
