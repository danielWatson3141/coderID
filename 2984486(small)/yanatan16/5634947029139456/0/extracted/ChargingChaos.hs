...
findFirstFull :: Int -> [(Flow, [a])] -> Solution
findFirstFull n [] = NotPossible
findFirstFull n ((f,as):fas)
  | n == length as = MinSwitches (countOnes f)
  | otherwise = findFirstFull n fas

invertFlowTable :: [((Int, Int), Flow)] -> [(Flow, [(Int, Int)])]
invertFlowTable = sortBy cmp . toList . foldr f empty
  where
    cmp (f,_) (g,_) = compare (countOnes f) (countOnes g)
