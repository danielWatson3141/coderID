...
class FileWrapper:
    ...
    def readlines(self, num):
        return [self.readline() for _ in range(num)]
...
class GCJ:
    ...
    def parse(inFile):
        ...
        searchEngines = [inFile.readline().strip() for z in range(N)]
        Q = int(inFile.readline())
        queries = [inFile.readline().strip() for z in range(Q)]
        return [searchEngines, queries]

    def solve([searchEngines, queries]):
        N = len(searchEngines)
        dict = {searchEngines[i]: i for i in range(N)}
...
"""
    ...
    def __init__(self, reader, solver, directory, question):
        ...
        sys.stderr.write("GCJ Wrapper initiated.")
        sys.stderr.write("Reading command line arguments")
        try:
        ...
        if (self.job == "run"):
            sys.stderr.write("Creating " + self.outfile + " from " + self.infile)
        else:
            sys.stderr.write("Testing that " + self.outfile + " would create " + self.infile)
            sys.stderr.write("Storing output in " + self.errfile + " otherwise")
    def run(self):
        data = self.read()
        if (self.multi == 1):
