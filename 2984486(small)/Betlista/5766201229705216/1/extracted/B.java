...
public class B {
	...
	private static int tryRoot(ArrayList<Integer>[] g, int from, int prev) {
	...
	private static int count(ArrayList<Integer>[] g, int from, int prev) {
		int children = foundChildren(g, from, prev);
		if (children == 0) {
			return 1;
		}
	...
	private static int foundChildren(ArrayList<Integer>[] g, int from, int prev) {
		int res = 0;
		for (int next : g[from]) {
			if (next != prev) {
				++res;
