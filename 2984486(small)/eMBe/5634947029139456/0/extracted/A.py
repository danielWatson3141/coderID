...
def solve():
    n, l = gcj.tokens(2, int)
    flows1 = gcj.tokens(n, lambda s: int(s, 2))
    flows2 = gcj.tokens(n, lambda s: int(s, 2))
    res = None
    for i in xrange(n):
        x = flows1[i] ^ flows2[0]
        f2 = [q^x for q in flows2]
        if set(f2) == set(flows1):
            c = bin(x).count('1')
            if res is None or res > c:
                res = c
    return res if res is not None else "NOT POSSIBLE"
