...
bool solve(vi a)
{
	...
	rep(i,n)
	{
		...
		int pai=i;
		if (pai>=ai) pai-=n;
		if (0<ai&&ai<n-1) cnt+=1/exp((ai-1)-pai);
	}
	double good=0;
	rep(i,n) rep(j,n)
	{
		...
		int pai=j;
		if (pai>=ai) pai-=n;
		if (0<ai&&ai<n-1) good+=1/exp((ai-1)-pai);
	}
	good/=n;
	bool yes=cnt>good;
	return yes;
...
//	debug=true;
	if (debug)
	{
		...
		rep(i,n)
		{
			rep(j,n)
			{
				cerr<<i<<","<<j<<":"<<floor(cnt[i][j]-pow(n,n-1)+0.5)<<endl;
			}
		}
		rep(i,n)
		{
			rep(j,n)
			{
				cerr<<i<<","<<j<<":"<<floor(cnt[j][i]-pow(n,n-1)+0.5)<<endl;
			}
		}
		prt(cntA);
		vi a;
