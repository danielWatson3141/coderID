...
int size(vvi& adj,map<pii,int>& s,int u,int p)
{
	if (!s.count(mp(u,p)))
	{
		int ans=1;
		...
		{
			int v=adj[u][i];
			if (v!=p) ans+=size(adj,s,v,u);
		}
		return ans;
	}
	return s[mp(u,p)];
}
int calc(vvi& adj,map<pii,int>& c,map<pii,int>& s,int u,int p)
{
	if (!c.count(mp(u,p)))
	{
		int cutAll=size(adj,s,u,p)-1;
		int ans=cutAll;
		rep(i,sz(adj[u]))
		{
			int v=adj[u][i];
			...
			rep(j,i)
			{
				int w=adj[u][j];
				if (w==p) continue;
				int get=calc(adj,c,s,v,u)+calc(adj,c,s,w,u)+cutAll-size(adj,s,v,u)-size(adj,s,w,u);
				cmin(ans,get);
			}
		}
		c[mp(u,p)]=ans;
