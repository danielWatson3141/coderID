...
int GetLayer2(Tree& t, int topnode, int li0, int li1, int parent)
	;

int GetLayer(Tree& t, int topnode, int parent)
{
	Node& n = t.nodes[topnode];
	int m = 1;
	for (size_t i = 0; i < n.ns.size(); ++i)
	{
		for (size_t j = i + 1; i < n.ns.size(); ++i)
		{
			int x = GetLayer2(t, topnode, i, (i+1)%n.ns.size(), parent);
			m = (std::max)(m, x);
		}
	}
	return m;
...
int GetLayer2(Tree& t, int topnode, int li0, int li1, int parent)
{
	...
	int ni0 = n.ni(li0);
	int ni1 = n.ni(li1);
	if (ni0 == parent || ni1 == parent)
		return 0;

	int ml0 = GetLayer(t, ni0, topnode);;
	int ml1 = GetLayer(t, ni1, topnode);

	return ml0 + ml1 + 1;
}

int GetChilds(Tree& t, int nd, int parent)
{
	Node& n = t.nodes[nd];
	if (n.ns.size() <= 2)
		return 1;

	vector<int> counts(n.ns.size(), 0);
	for (size_t i = 0; i < n.ns.size(); ++i)
	{
		if (n.ns[i] == parent)
			continue;

		counts[i] = GetChilds(t, n.ns[i], nd);
	}
...
int MaxTree2(Tree& t, int topnode)
{
	Node& n = t.nodes[topnode];
	if (n.ns.size() <= 1)
		return 1;

	vector<int> counts(n.ns.size());
	for (size_t i = 0; i < n.ns.size(); ++i)
	{
		counts[i] = GetChilds(t, n.ns[i], topnode);
	}
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	{
		maxtree = (std::max)(MaxTree2(t, i), maxtree);
	}

	int dels = t.Nodecnt() - maxtree;
	OUTANS( << CaseHead(caseIdx+1) << dels << endl );
}
