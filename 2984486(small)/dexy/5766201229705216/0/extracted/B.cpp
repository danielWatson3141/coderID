...
int f(int u, int pu) {
  ...
  REP(ie, SIZE(e [u])) {
    ...
    total += tmp [ie];
    //cout << u << "," << pu << ": " << ie << "," << v << "-" << tmp [ie] << "   " << total << endl;
  }
  ret = min(ret, total);
  REP(ie1, SIZE(e [u])) {
    int v1 = e [u] [ie1];
    if (v1 != pu) {
      FOR(ie2, ie1 + 1, SIZE(e [u])) {
        ...
        if (v2 != pu) {
          ...
          //cout << u << "," << pu << "--->" << ie1 << "," << v1 << "," << f (u, v1) << "   " << ie2 << "," << v2 << "," << f(u, v2) << "   " << total << "," << tmp [ie1] << "," << tmp [ie2] << "--->" << cur<<endl;
          ret = min(ret, cur);
        }
      }
    }
...
int main() {
  ...
  FOR(test, 1, numTests + 1) {
    ...
    //REP(u, n + 1) { cout << u << "->" ;REP(v, n + 1) cout << v << ":" << dp [u] [v] << "  " ; cout << endl ;}
