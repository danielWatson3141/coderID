...
vvi G;
int dfs(int at, int par, int bm) {
    int z = 1;
    FOR(i,n) if ((bm&(1<<i)) and g[at][i] and i!=par) {
        G[at].pb(i);
        z += dfs(i, at, bm);
    }
    return z;
...
bool isfull(int at) {
    if (sz(G[at]) == 0) return true;
    if (sz(G[at]) == 2) {
        return isfull(G[at][0]) and isfull(G[at][1]);
    }
    return false;
}
...
int main() {
    ...
    FOR1(cas,T) {
        ...
        int big = 0;
        FOR(bm,(1<<n)) {
            if (intbc(bm) <= big) continue;
            FOR(i,n) if (bm&(1<<i)) {
                G = vvi(n);
                int z = dfs(i, -1, bm);
                if (z != intbc(bm)) continue;
                if (isfull(i)) {
                    big = z;
                }
            }
