...
namespace RAVE
{
	public static class Utility
	{
		...
		{
			return b < 0 ? a > 0 ? (a - 1) / b - 1 : a / b : a < 0 ? (a + 1) / b - 1 : a / b;
		}

		public static long Modulus(long a, long b)
		{
			long result = a % b;
			return (b < 0 ? result > 0 : result < 0) ? result + b : result;
		}
		...
		class BinarySearchCompare<T> : IComparer<T>
		{
			internal Func<T, int> CompareDelegate;
			public int Compare(T x, T y) { return CompareDelegate(x); }
		}

		/// <summary>
		/// Do a binary search given a delegate that compares array values against the search value.
		/// Return the index if found.  Return negative number ~(index of first larger) if not found.
		/// </summary>
		public static int BinarySearch<T>(this T[] array, Func<T, int> compare)
		{
			return Array.BinarySearch(array, default(T), new BinarySearchCompare<T>() { CompareDelegate = compare });
		}

		/// <summary>
		/// Return the item with the maximum selected value, or default(T) for no items.
		/// </summary>
		public static T MaxBy<T, C>(this IEnumerable<T> source, Func<T, C> selector)
		{
			Comparer<C> comparer = Comparer<C>.Default;
			bool first = true;
			T maxItem = default(T);
			C maxValue = default(C);
			foreach (var current in source)
			{
				C currentValue = selector(current);
				if (first || comparer.Compare(currentValue, maxValue) > 0)
				{
					maxItem = current;
		...
		/// <summary>
		/// Get the prime factors of n (not the prime factorization of n).
		...
		/// <summary>
		/// Return the floor of log2 of the given value.  Throw if the value is less than 1.
		///    v       FloorLog2(v)
		///    -----   ------------
		///    1       0
		///    2..3    1
		...
		///    8..15   3
		/// </summary>
		public static int FloorLog2(long v)
		{
			if (v <= 0)
		...
		public static ulong[] ReadULongs(this TextReader input)
		{
			return input.ReadStrings().Select(s => ulong.Parse(s)).ToArray();
		}
		...
		public static long FloorMultiple(long a, long m)
		{
			long r = a % m;
			return r < 0 ? a - r - m : a - r;
		}

		public static long CeilingMultiple(long a, long m)
		{
			long r = a % m;
			return r > 0 ? a - r + m : a - r;
		}
		...
		/// <summary>
		/// Return Floor(Sqrt(n)) for non-negative n.  Throw DivideByZeroException for negative n.
		/// </summary>
		public static BigInteger ISqrt(this BigInteger n)
		{
			var ba = n.ToByteArray();
			var lb = ba[ba.Length - 1];
			var sh = ((ba.Length - 1) << 2) + (lb >= 64 ? 4 : lb >= 16 ? 3 : lb >= 4 ? 2 : lb >= 1 ? 1 : 0);

			var prev = BigInteger.One << sh;
			var curr = (prev + n / prev) >> 1;
