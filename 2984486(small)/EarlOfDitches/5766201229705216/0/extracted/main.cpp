...
pair<int,int> dfs(int v)
{
	...
	dp[0][0] = 0;
	dp[0][1] = dp[0][2] = 10000000;
	for(int i=1;i<=cnt;++i)
	{
		
		dp[i][0] = dp[i][1] = dp[i][2] = 10000000;
		
		dp[i][0] = min(dp[i][0],dp[i-1][0] + sz[i-1]);

		dp[i][1] = min(dp[i][1],
				min(dp[i-1][1] + sz[i-1],dp[i-1][0]+data[i-1]));
		
		dp[i][2] = min(dp[i][2],
				min(dp[i-1][2] + sz[i-1],dp[i-1][1]+data[i-1]));
	}
