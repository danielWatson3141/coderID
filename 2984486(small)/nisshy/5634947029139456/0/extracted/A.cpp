...
template<class T, class U>
ostream &operator<<(ostream &os, const unordered_map<T, U> &c) {
	return dump_range(os, c.cbegin(), c.cend());
}
...
string solve() {
	...
	for(auto &e : outlets) cin >> e;

	vector<string> devices(n);
	for(auto &e : devices) cin >> e;
	...
	for(int flip = 0; flip < (1 << l); ++flip) {
		const int t = __builtin_popcount(flip);
		if(ans <= t) continue;

		unordered_map<string, int> rest(cnt);
		for(int i = 0; i < n; ++i) {
			string tmp = "";
			for(int j = 0; j < l; ++j) {
				if((flip >> j) & 1) {
	...
	return ans == INT_MAX ? "NOT POSSIBLE" : to_string(ans);
}
