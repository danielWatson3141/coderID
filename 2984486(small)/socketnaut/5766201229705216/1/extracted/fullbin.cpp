...
ifstream fin("fullbinlarge.in");
ofstream fout("fullbinlarge.out");

int N, X, Y;
vector<int> edg[10005];

int sizmem[1005][1005];
int minmem[1005][1005];

int size(int root, int par){
    if(sizmem[root][par]!=0) return sizmem[root][par];
    ...
    return sizmem[root][par] = ans;

}

int min_cost(int root, int par){
    if(minmem[root][par]!=0) return minmem[root][par];
...
int main(){
    ...
    for(int t=0; t<T; t++){
        ...
        memset(sizmem, 0, sizeof(sizmem));
        memset(minmem, 0, sizeof(minmem));
