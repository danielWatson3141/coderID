...
namespace gcj_rotate
{
    public partial class Form1 : Form
    {
      ...
      var sr = new StreamReader("A-large.in ");
        ...
        {
            var sr = new StreamReader("B-small-attempt0.in ");
            var sw = new StreamWriter("B-small-attempt0.out");
            int T = Convert.ToInt32(sr.ReadLine());
            for (int t = 0; t < T; t++)
            {
                int N = Convert.ToInt32(sr.ReadLine());
                ...
                {
                    var vals = sr.ReadLine().Split(' ');
                    edges.Add(new Point(Convert.ToInt32(vals[0]), Convert.ToInt32(vals[1])));
                }
                int deg2 = 0;
                int del = 0;
                bool deleted=true;
                while (deleted)
                {
                    ...
                    { 
                        ...
                        for (int j = 0; j < N - 1; j++)
                        {
                            if (edges[j].X == i || edges[j].Y == i) deg++;
                        }
                        if (deg == 2) deg2++;
                        if (deg > 3)
                        {
                   /*         int min = int.MaxValue;
                            ...
                            for (int k = 0; k < N - 1; k++)
                            {
                                if (edges[k].X == i)
                                {
                                    int cur = getst(i, edges[k].Y, edges);
                                    if (cur < min) { min = cur; mina = edges[k].Y; }
                                }
                            }
                            List<int> delset=new List<int>();
                            delset.Add(mina);
                            while (true)
                            {
                                for (int j = 0; j < N - 1; j++)
                                {
                                    if (delset.Contains(edges[j].X))
                                    {
                                        if(edges[j].Y!=i) delset.Add(edges[j].Y);
                                        edges[j]= new Point(0,0);
                                    } else
                                        if (delset.Contains(edges[j].Y))
                                        {
                                            if (edges[j].X != i) delset.Add(edges[j].X);
                                            edges[j] = new Point(0, 0);
                                        }
                                }
                            }
                            ...
                            deleted = true;
                            break;*/
                        } // delete smallest subtree;
                    }
                }
                if (deg2 == 0) sw.WriteLine("Case #" + (t + 1).ToString() +": "+ (del+1).ToString()); else
                    sw.WriteLine("Case #" + (t + 1).ToString() +": "+ (deg2-1+del).ToString());
