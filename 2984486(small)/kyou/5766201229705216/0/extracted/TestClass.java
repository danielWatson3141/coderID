...
public class TestClass {
	...
	public static void main (String[] args) throws IOException {
		...
		for(int i=0; i<caseNum; i++) {
			...
			for(int j=0; j<nodeNum-1; j++) {
				...
				pairs.add(testClass.new Pair(Integer.parseInt(tokens[0]), Integer.parseInt(tokens[1])));
			}
			
			writer.write("Case #" + (i+1) + ":");
			writer.write(" " + testClass.pb(pairs, nodeNum));
			writer.write("\n");
	...
	public int pb(List<Pair> pairs, int nodeNum) {
		int ret = 0;
		int[] nodeCounts = new int[nodeNum];
		Arrays.fill(nodeCounts, 0);
		
		for(Pair pair : pairs) {
			nodeCounts[pair.one-1]++;
			nodeCounts[pair.two-1]++;
		}
		...
		while(true) {
			...
			for(int i=0; i<nodeNum; i++) {
				if(nodeCounts[i] == 1) {
					...
					while(true) {
						for(Pair pair : pairs) {
							if(pair.one == found || pair.two == found) {
								tmpCount++;
