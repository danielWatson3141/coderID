...
public class A {
	...
	class Config {
		...
		public static final String BASE_DIR = "data\\r1a\\A" ;
//		public static final String IN_FILE = "sample.txt";
	...
	class ParallelSolver implements Runnable {
		...
		@Override
		public void run() {
			Comparator<char[]> cmp = new Comparator<char[]>() {
				@Override
			...
			Arrays.sort(device, cmp);
			long ans = Long.MAX_VALUE;
			for(int i = 0; i < N; i++) {
				long flip = 0;
				for(int j = 0; j < L; j++) {
					if(outlet[i][j] != device[0][j]) {
						flip |= 1L << j;
						for(int k = 0; k < N; k++) {
							outlet[k][j] = (char)('1' + '0' - outlet[k][j]);
						}
					}
				}
				
				LOOP: do {
					char[][] cs = outlet.clone();
					Arrays.sort(cs, cmp);
					for(int j = 0; j < N; j++) {
						if(!Arrays.equals(cs[j], device[j])) {
							break LOOP;
						}
					}
			...
			out.println(ans == Long.MAX_VALUE ? "NOT POSSIBLE" : ans);
		}
