...
def solve_one(f):
    ...
    def solve(outlets, devices, n, s):
        ...
        if diff == 0 and new_diff == 0:
            return min(solve(outlets, devices, n-1, s), solve(new_outlets, devices, n-1, s+1))
        elif new_diff == 0:
            print("-----")
            print(as_bin(new_outlets), as_bin(devices), sep="\n")
            return solve(new_outlets, devices, n-1, s+1)
        elif diff == 0:
            return solve(outlets, devices, n-1, s)
        else:
            return "NOT POSSIBLE"

    return solve(O, D, L, 0)
