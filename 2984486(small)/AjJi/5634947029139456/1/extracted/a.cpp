...
int common(vector< vector<long long int> > a) {
	vector<long long int> in = a[0];
	int best = 1000000;
	REP(i, in.size()) {
		int c = 1;
...
long long int todec(string s) {
	long long int d = 0LL, p = 1LL;
	for(int i = s.size() - 1; i >= 0; i--) {
		d += (long long)(s[i]-'0') * (long long)p;
		p *= 2;
	}
	return d;
}
int main()
{
   ...
   freopen("A-large3.out", "w", stdout);
	...
	REP(k, cases) {
		...
		vector<long long int> flows, devices;
		getline(cin, s);
		stringstream st;
		st<<s;
		string tmp;
		...
		vector< vector<long long int> > diffs;
		REP(i,flows.size()) {
