...
public class B {
    ...
    private static final Map<String, Integer> cache2 = new HashMap<String, Integer>();
    ...
    private static int countRoot(Integer rootCandidate, Map<Integer, List<Integer>> graph) {
        ...
        String xx = "" + rootCandidate + conns;
        if (cache2.containsKey(xx)) {
            return cache2.get(xx);
        }
        //System.out.print(".");
        int result;
        int c = conns.size();
        if (c == 0) {
            ...
            result = sizeOf(rootCandidate, graph) - maxSave - 1;
        } else {
            throw new IllegalStateException("!!");
        }
        cache2.put(xx, result);
        return result;
    }

    private static int sizeOf(int v, Map<Integer, List<Integer>> graph) {
        String x = "" + v + graph.get(v);
