...
int main(int argc, char *argv[]) {
	...
	while (T--) {
		...
		for (int x = 0; x < (1 << half); ++x) {
			...
			sort(c + 1, c + n + 1);
			ll cur = Hash();
			h.insert(pair(cur, pair(__builtin_popcount(x), x)));
		}
		sort(b + 1, b + n + 1);
		int ans = l + 1;
		for (int x = 0; x < (1 << l - half); ++x) {
			for (int i = 1; i <= n; ++i) {
				c[i] = b[i];
				for (int j = half; j < l; ++j)
					if (x >> j - half & 1) c[i] ^= 1LL << j;
			}
			sort(c + 1, c + n + 1);
			ll cur = Hash();
			it = h.lower_bound(pair(cur, pair(0, 0)));
			if (it != h.end() && it->first == cur) {
				if (check(it->second.second + ((ll)x << half)))
					ans = min(ans, __builtin_popcount(x) + it->second.first);
			}
		}
