...
bignum cal2(const vector<int>& a){
    int n = a.size();
    bignum ret = 0;
    for (int i=0; i<n; i++){
        int cc = 0;
...
void gao(int cas){
    ...
    printf("Case #%d: ",cas);
    if ( res>=fact[n]*995/1000/2 ) printf("GOOD\n");
    else printf("BAD\n");
...
void test(){
    ...
    while ( n<100000 ){
        ...
        //for  (int i=0; i<a.size(); i++){ int j = rand()%a.size(); swap(a[i], a[j]); }
...
int main(){
    fact[0] = 1;
    for (int i=1; i<1220; i++) fact[i] = fact[i-1]*i;
    //test();
