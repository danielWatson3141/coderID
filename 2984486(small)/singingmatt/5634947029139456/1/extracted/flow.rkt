...
;; intersection : [List X] [List Y] -> [List X or Y]
(define (intersection lox loy)
  (filter (λ (y) (member y lox)) loy))
(check-expect (intersection '(1 2 3 4 5) '(3 4 5 6)) '(3 4 5))



;; main : [List Device] x [List Outlet] -> Outpuet
(define (main lod loo)
  (local ((define (helper lod acc)
            (if (empty? lod) acc
                (local ((define maybe-difs (differences (first lod) loo)))
                  (helper (rest lod) (intersection acc maybe-difs))))))
           (local ((define h (helper (rest lod) (differences (first lod) loo))))
             (if (empty? h) "NOT POSSIBLE"
                 (min-dif h)))))
...
(test)
(write-file "aoutbig.txt" (file->output (rest (read-words/line "A-large.in"))))