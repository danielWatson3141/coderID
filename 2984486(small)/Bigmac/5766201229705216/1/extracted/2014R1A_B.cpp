...
char readChar()
{
	...
	while(in_v == ' ' || in_v == 0x0d || in_v == 0x0a || in_v == 0x00)
	{
		...
		if(in_v != 0x0d && in_v != 0x0a && in_v != 0x00)
		{
			return in_v;
		}
	}
...
vector< int > color;
//2014 Qual QE
int remove_cost(int r, bool root)
{
    color[r] = 1;
    int cost = 0;
    for(int i = 0; i < edges[r].size();i++)
    {
        if(color[edges[r][i]] == 0)
        {
            cost += remove_cost(edges[r][i], false);
        }
    }
    color[r] = 0;
    return cost + 1;
}
int tree_cost(int r, bool root)
{
    int e_bnum = 1;
    if(root)
    {
    ...
    color[r] = 1;
    int cost = 0;
    if(edges[r].size() == e_bnum)
    {
        cost = 0;
    }
    else if(edges[r].size() == e_bnum + 1)
    {
        for(int i = 0; i < edges[r].size();i++)
        {
            if(color[edges[r][i]] == 0)
    ...
    else if(edges[r].size() == e_bnum + 2)
    {
        for(int i = 0; i < edges[r].size();i++)
        {
            if(color[edges[r][i]] == 0)
            {
                cost += tree_cost(edges[r][i], false);
            }
        }
    }
    else if(edges[r].size() >= e_bnum + 3)
    {
        int cost1 = INT_MAX;
        int cost2 = INT_MAX;
        int cand1 = -1;
        ...
        for(int i = 0; i < edges[r].size();i++)
        {
            if(color[edges[r][i]] == 0)
            {
                int tc = tree_cost(edges[r][i], false) - remove_cost(edges[r][i], false);
                if(tc <= cost1)
                {
                    cost2 = cost1;
                    cand2 = cand1;
