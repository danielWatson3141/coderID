...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
        ...
        auto cmp = [&]() -> bool{
            REP(i, in_n)
            {
                if(strcmp(outl[i].c_str(), devi[i].c_str()) != 0)
                {
                    return false;
                }
            }
        ...
        sort(outl.begin(), outl.end(), [](const string& a, const string& b)->bool{
            return strcmp(a.c_str(), b.c_str()) >0;
        });
        sort(devi.begin(), devi.end(), [](const string& a, const string& b)->bool{
            return strcmp(a.c_str(), b.c_str()) >0;
        });
        REP(i, in_n)
        {
            printf("\\");
                printf("%s=%s", devi[i].c_str(), outl[i].c_str());
            printf("\\");
        }
		auto solve = [&]() -> int{
            for(int i = 0; i < pow(2,in_l); i++)
            {
                int cnt = 0;
                REP(j, in_l)
                {
        ...
        if(min_solve > in_l)
        {
            fprintf(fo, "Case #%d: NOT POSSIBLE", it_t);
        }
        else
        {
